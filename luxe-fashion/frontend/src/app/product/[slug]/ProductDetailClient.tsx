  ...
  const selectedColorLabel = selectedColor
    ? getFriendlyColorName(
        ...
        selectedVariant?.colorHex ??
          (colors.find((c: any) => c.color === selectedColor) as { colorHex?: string })
            ?.colorHex,
      )
    : "";
                    ...
                    >
                      {isActive && (
                        <Check
                          ...
                          style={{
                            color:
                              v.colorHex?.toUpperCase() === "#FFFFFF" ||
                              v.colorHex?.toUpperCase() === "#F5F5F0"
                                ? "#201610"
                                : "#ffffff",
                          }}
                        />
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs tracking-widest uppercase text-brand-500">
                  Size
                  {selectedSize && (
                    <span className="text-brand-900 dark:text-white ml-1">
                      : {selectedSize}
                    </span>
                  )}
                </p>
      ...
      <div className="grid lg:grid-cols-2 gap-16">
        ...
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className={`h-${i === 1 ? 4 : i === 2 ? 10 : 8} skeleton ${i === 2 ? "w-3/4" : i === 5 ? "w-full" : "w-1/2"}`}
            />
          ))}
        </div>
      </div>
