...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  const accordion = (id: string, label: string, content: React.ReactNode) => (
    <div className="border-t border-brand-100 dark:border-brand-800">
      ...
      {openAccordion === id && (
        <div className="pb-4 text-sm text-brand-600 dark:text-brand-400 leading-relaxed">{content}</div>
      )}
    </div>
  );
            ...
            {displayImages.map((img: any, i: number) => (
              <button key={i} onClick={() => setMainImg(i)}
                className={`relative aspect-square overflow-hidden border-2 transition-all ${
                  mainImg === i ? "border-brand-900 dark:border-white" : "border-transparent hover:border-brand-300"
                }`}>
        ...
        <div className="flex flex-col gap-5">
          ...
          {colors.length > 0 && (
            <div>
              <p className="text-xs tracking-widest uppercase text-brand-500 mb-3">
                Color
                {selectedColorLabel && (
                  <span className="text-brand-900 dark:text-white ml-1 normal-case font-normal">: {selectedColorLabel}</span>
                )}
              </p>
              <div className="flex gap-2 flex-wrap">
                {colors.map((v: any) => {
                  ...
                  return (
                    ...
                    >
                      {isActive && (
                        <Check size={12} className="absolute inset-0 m-auto drop-shadow"
                          style={{ color: v.colorHex?.toUpperCase() === "#FFFFFF" || v.colorHex?.toUpperCase() === "#F5F5F0" ? "#201610" : "#ffffff" }} />
                      )}
                    </button>
                  );
                })}
          ...
          {sizes.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs tracking-widests uppercase text-brand-500">
          ...
          <div className="flex gap-3">
            <div className="flex items-center border border-brand-200 dark:border-brand-700">
              ...
              <input type="number" min={1} max={maxQuantity > 0 ? maxQuantity : 1} value={quantity}
                ...
                aria-label="Quantity" />
              <button type="button" onClick={() => setQuantity((q) => clampQuantity(q + 1))} disabled={!inStock || atMaxQuantity}
                className="w-11 h-11 flex items-center justify-center text-brand-500 hover:text-brand-900 dark:hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
                <Plus size={14} />
              </button>
            </div>
            <button type="button" onClick={handleAdd} disabled={!inStock || (hasVariants && maxQuantity === 0)}
            ...
            <button
              ...
              className={`w-11 h-11 border flex items-center justify-center transition-colors ${
                isWishlisted ? "border-red-300 text-red-500 bg-red-50 dark:bg-red-900/10"
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          <div>
            <div className="text-center p-8 border border-brand-100 dark:border-brand-800">
              <p className="font-serif text-6xl font-light text-brand-900 dark:text-white mb-2">{Number(p.avgRating).toFixed(1)}</p>
            ...
            <form onSubmit={handleReviewSubmit} className="mt-6 space-y-4">
              ...
              <div>
                <p className="text-xs tracking-widests uppercase text-brand-500 mb-2">Rating</p>
          ...
          <div className="lg:col-span-2 space-y-6">
            {p.reviews?.length === 0 && <p className="text-brand-500 text-sm py-8">No reviews yet. Be the first to review this product.</p>}
            {p.reviews?.map((r: any) => (
