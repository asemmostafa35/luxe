...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ✅ FIX 2 — Variant Colors Not Displaying
  //
  // ROOT CAUSE: The variant `color` and `colorHex` fields ARE saved to the DB
  // (confirmed by seed data). The issue is in how variants are fetched and
  // mapped. `getProducts` (list view) uses:
  //   variants: { select: { size, color, colorHex, stock } }
  // while `getProduct` (detail page) uses:
  //   variants: { where: { isActive: true } }        ← full object ✓
  //
  // The detail page query is correct. The real bug is that newly admin-created
  // products set `isActive` on variants based on what the frontend sends, but
  // the frontend `addVariant()` function didn't set `isActive: true`, so new
  // variants were saved with Prisma's default (which is true per schema) — so
  // that part is fine.
  //
  // The ACTUAL display bug: the `colors` deduplication uses:
  //   [...new Map(p.variants?.filter(v => v.color).map(v => [v.color, v])).values()]
  // This is correct. But `v.colorHex` can be `null` or `undefined` when the
  // admin didn't fill it in. The swatch button gets `backgroundColor: undefined`
  // which renders as a transparent circle — INVISIBLE on white background.
  //
  // FIXES:
  //  1. Default colorHex to a visible neutral (#6b7280) when missing.
  //  2. Show color name as a text label fallback when hex is absent.
  //  3. Use `useEffect` to auto-select the first available color on load
  //     instead of starting with null, so the UI is never in an "unselected"
  //     state that hides the swatches (they render but nothing is highlighted).
  // ─────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    if (!product) return;
    const firstColor =
      product.variants?.filter((v: any) => v.color && v.stock > 0)?.[0]
        ?.color ?? null;
    const firstSize =
      product.variants?.filter((v: any) => v.size && v.stock > 0)?.[0]?.size ??
      null;
  ...
  if (!product)
    return <div className="p-16 text-center">Product not found</div>;
  ...
  // ✅ FIX: sizes from ALL variants (not just those matching selectedColor)
  //    so the size grid is always populated
  const sizes: string[] = [
    ...new Set<string>(
      (p.variants ?? [])
        .filter((v: any) => v.size)
  ...
  // ✅ FIX: deduplicated color list — guaranteed to have colorHex fallback
  const colors: Array<{ color: string; colorHex: string; stock: number }> = [
    ...new Map<string, any>(
      (p.variants ?? [])
        ...
        .map((v: any) => [
          ...
          {
            color: v.color,
            // ✅ If colorHex is missing/null/empty, fall back to a visible neutral
            colorHex:
              v.colorHex && v.colorHex.trim() !== "" ? v.colorHex : "#6b7280",
            stock: v.stock,
          },
        ]),
    ).values(),
  ...
  const inStock = selectedVariant
    ...
    : ((p.variants ?? []).some((v: any) => v.stock > 0) ?? true);
    ...
    <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-8">
      ...
      <div className="grid lg:grid-cols-2 gap-8 xl:gap-16">
          ...
          <div className="flex-1 relative aspect-[3/4] overflow-hidden bg-brand-50 dark:bg-brand-800">
            {p.images?.[mainImg] && (
              <Image
                ...
                alt={p.images[mainImg].alt || p.name}
                fill
        ...
        <div className="flex flex-col gap-5">
            ...
            {Number(p.avgRating) > 0 && (
              <div className="flex items-center gap-2 mb-4">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_: any, i: number) => (
                    <Star
                      ...
                      size={14}
                      className={
                        i < Math.round(Number(p.avgRating))
                ...
                <span className="text-sm text-brand-500">
                  ...
                  {p._count?.reviews || p.reviews?.length || 0} reviews)
                </span>
              </div>
            )}
          ...
          {/* ✅ FIX: Colors section — renders correctly even when colorHex is missing */}
          {colors.length > 0 && (
            <div>
                  ...
                  <button
                    ...
                    className={`relative w-8 h-8 rounded-full border-2 transition-all ${
                      selectedColor === v.color
                        ? "border-brand-900 dark:border-white scale-110 shadow-md"
                        : "border-brand-200 dark:border-brand-700 hover:scale-105"
                  ...
                  >
                    {selectedColor === v.color && (
                      <Check
                        ...
                        style={{
                          // White check on dark colours, dark check on light ones
                          color: isLightColor(v.colorHex)
                            ? "#1a1a1a"
                            : "#ffffff",
                        }}
                      />
              ...
              {/* ✅ FIX: Text fallback — always shows color names below swatches
                  so the user can read them even if swatches are hard to distinguish */}
                ...
                {colors.map((v) => (
                  <span
                    ...
                    className={`text-xs cursor-pointer transition-colors ${
                      selectedColor === v.color
                        ? "text-brand-900 dark:text-white font-medium underline underline-offset-2"
          ...
          <div className="flex gap-3">
            <div className="flex items-center border border-brand-200 dark:border-brand-700">
              <button
                ...
                className="w-11 h-11 flex items-center justify-center text-brand-500 hover:text-brand-900 dark:hover:text-white transition-colors"
              >
                <Minus size={14} />
              </button>
            ...
            <button
              onClick={handleShare}
              className="w-11 h-11 border border-brand-200 dark:border-brand-700 flex items-center justify-center text-brand-500 hover:border-brand-900 dark:hover:border-white hover:text-brand-900 dark:hover:text-white transition-colors"
              aria-label="Share"
            >
              <Share2 size={16} />
            </button>
      ...
      <section className="mt-20">
        <h2 className="font-serif text-3xl font-light text-brand-900 dark:text-white mb-8">
          Customer Reviews
        </h2>
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            {p.reviews?.length === 0 && (
              <p className="text-brand-500 text-sm py-8">
            ...
            {p.reviews?.map((r: any) => (
              <div
                key={r.id}
                className="border-b border-brand-100 dark:border-brand-800 pb-6"
              >
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    {r.title && (
                      <p className="font-medium text-sm text-brand-900 dark:text-white">
                        {r.title}
                      </p>
                    )}
                  </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// ✅ Helper: determine if a hex colour is light (for check-mark contrast)
// ─────────────────────────────────────────────────────────────────────────────
