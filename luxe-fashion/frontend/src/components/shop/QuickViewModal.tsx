...
export default function QuickViewModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={closeQuickView}>
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
      <div className="relative bg-white dark:bg-brand-950 max-w-3xl w-full max-h-[90vh] overflow-y-auto animate-scale-in shadow-2xl"
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="relative aspect-[3/4] bg-brand-50 dark:bg-brand-900">
            {p.images?.[imgIdx] && (
              <Image src={p.images[imgIdx].url} alt={p.name} fill className="object-cover" />
            )}
            {p.images?.length > 1 && (
              <>
          ...
          <div className="p-6 md:p-8 flex flex-col gap-4">
            <div>
              <p className="label-small text-brand-500 mb-2">{p.category?.name}</p>
              <h2 className="font-serif text-2xl font-light text-brand-900 dark:text-white mb-2">{p.name}</h2>
              {p.avgRating > 0 && (
                <div className="flex items-center gap-1.5 mb-3">
                  {Array.from({ length: 5 }).map((_: any, i: number) => (
                    <Star key={i} size={12} className={i < Math.round(p.avgRating) ? 'fill-gold-500 text-gold-500' : 'text-brand-200 dark:text-brand-700'} />
                  ))}
                  <span className="text-xs text-brand-500">({p._count?.reviews || 0})</span>
                </div>
              )}
              <div className="flex items-center gap-3">
                <span className="text-2xl font-medium text-brand-900 dark:text-white">${Number(p.price).toFixed(2)}</span>
                {p.comparePrice && <span className="text-brand-400 line-through">${Number(p.comparePrice).toFixed(2)}</span>}
              </div>
            </div>

            <p className="text-sm text-brand-600 dark:text-brand-400 leading-relaxed line-clamp-3">{p.description}</p>

            {/* Colors */}
            {colors.length > 0 && (
              <div>
                <p className="text-xs tracking-widest uppercase text-brand-500 mb-2">Color{selectedColor && `: ${selectedColor}`}</p>
            ...
            {sizes.length > 0 && (
              <div>
                <p className="text-xs tracking-widest uppercase text-brand-500 mb-2">Size{selectedSize && `: ${selectedSize}`}</p>
            ...
            <div className="flex gap-3 mt-2">
              <button onClick={handleAdd} className="flex-1 btn-primary text-xs flex items-center justify-center gap-2">
                <ShoppingBag size={14} /> Add to Bag
              </button>
              <button
                onClick={() => { wishlist.toggle({ productId: p.id, name: p.name, price: Number(p.price), image: p.images?.[0]?.url, slug: p.slug }); }}
            ...
            <Link href={`/product/${p.slug}`} onClick={closeQuickView}
